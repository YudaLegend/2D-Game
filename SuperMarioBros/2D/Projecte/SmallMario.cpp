...
void SmallMario::init(const glm::ivec2& tileMapPos, ShaderProgram& shaderProgram)
{
	bJumping = false;
	direction = false;
	firstJump = false;
...
void SmallMario::update(int deltaTime, float scroll)
{
...
void SmallMario::setStartMode() {
	starmode = true;
}

void SmallMario::setJump() {
	bJumping = true;
	jumpAngle = 0;
	height = 25;
	startY = posPlayer.y;
...
void SmallMario::setTileMap(TileMap* tileMap)
{
	map = tileMap;
}

void SmallMario::setPosition(const glm::vec2& pos)
{
	posPlayer = pos;
	sprite->setPosition(glm::vec2(float(tileMapDispl.x + posPlayer.x), float(tileMapDispl.y + posPlayer.y)));
}

bool SmallMario::collisionInt() {
	return map->collisionInt(posPlayer, glm::ivec2(16, 16), &posPlayer.y);
}

glm::ivec2 SmallMario::posInt() {
	return map->posInt(posPlayer, glm::ivec2(16, 16), &posPlayer.y);
}


glm::ivec2 SmallMario::getPosition() {
	return posPlayer;
}